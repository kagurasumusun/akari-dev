...
`data/apis/gettickcount.json` is the local authoritative implementation input for the first supported declaration. It records only facts consumed by this repository and pins the normalized upstream corpus record and commit that established them. `tools/generate_surface.py` deterministically produces:

- `generated/include/winbase.h` — the independently authored `GetTickCount` declaration and a 32-bit `DWORD` definition;
- `generated/link/gettickcount.link.json` — the documented CE 5.0 `Coredll.lib` linker requirement and explicit nulls for module/export facts not established by the evidence.
