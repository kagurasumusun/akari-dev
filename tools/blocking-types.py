...
MD_HEAD = """# Blocking types -- what one unpublished type hides
...
def write_md(path, njobs, reasons, top, held):
    md = path if os.path.isabs(path) else os.path.join(ROOT, path)
    os.makedirs(os.path.dirname(md), exist_ok=True)
    L = [MD_HEAD.rstrip(),
         "",
         "Undeclared documented names with a shipped header: **%d**" % njobs,
         "",
         "## Skip reasons",
         "",
         "| reason | count |",
    ...
    L += ["",
          ...
          "| --- | --- | --- | --- |"]
    for t, names in top[:60]:
        sn = sorted(names)
        ex = ", ".join("`%s`" % n for n in sn[:6])
        if len(sn) > 6:
            ex += ", ..."
        L.append("| `%s` | %d | %s | %s |" % (
            t, len(names),
            "`%s`" % held[t] if t in held else "**-- NOT RECORDED --**", ex))
...
def main():
    ...
    ap.add_argument("--md", default="docs/blocking-types.md",
                    help="markdown report; an unaudited gap is invisible, so "
                         "the report is a committed artifact, not console "
                         "output")
    a = ap.parse_args()
    ...
    for k, n in reasons.most_common(12):
        print("%-40s %d" % (k, n))
    print("\ntypes blocking the most documented names:")
    print("%-38s %6s  %-24s %s" % ("type", "blocks", "recorded in", "examples"))
    top = sorted(blocked_by.items(), key=lambda kv: -len(kv[1]))
    held = dfp.held_in_tree()
    for t, names in top[:a.top]:
        print("%-38s %6d  %-24s %s" % (
            t, len(names), held.get(t, "-- NOT RECORDED --"),
            ", ".join(sorted(names)[:5])))
    ...
    write_md(a.md, len(jobs), reasons, top, held)
