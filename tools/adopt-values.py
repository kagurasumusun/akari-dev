...
def emit_section(header, marker):
    ...
    L = []
    L.append('/* ================================================================== */')
    L.append(f'/* {marker} -- values confirmed against the')
    L.append(' * CeGCC-lineage w32api reference (R1; docs/clean-room.md v3')
    L.append(' * section 4, 2026-09-18 -- the value-check-only exception).')
    L.append(' * Every name below is documented by the official CE pages')
    L.append(' * WITHOUT a value (see the record comments and the held')
    L.append(' * ledger in this header); the value is a CE-era ABI fact')
    L.append(' * carried by the CE lineage itself.  Excluded trees are')
    L.append(' * never consulted (v3 section 3).  R1\'s')
