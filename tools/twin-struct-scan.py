#!/usr/bin/env python3
"""twin-struct-scan.py CORPUS_ROOT [--tsv docs/struct-audit-2026-09-18.tsv]
...
docs/twin-struct-scan-2026-09-18.tsv with statuses twin-match /
...
TSV = "docs/struct-audit-2026-09-18.tsv"
if "--tsv" in sys.argv:
...
DECL_CLOSE = re.compile(
    r"typedef\s+(?:struct|union)\s*(\w*)\s*\{(.*?)\}\s*([A-Z_]\w*)\s*(?:,[^;]*)?;", re.S)
...
def main():
    ...
    for name, hdr, pid in sorted(rows):
        ...
        for f in cands:
            ...
            for b in re.findall(r"<pre[^>]*>(.*?)</pre>", h, re.S):
                ...
                m = re.search(r"typedef\s+(?:struct|union)\s*(\w*)\s*\{(.*?)\}\s*" +
    ...
    path = "docs/twin-struct-scan-2026-09-18.tsv"
