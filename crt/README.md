> **Merged in-tree (M115).**  This directory is `kagurasumusun/wince-crt`
> at `5d21903`, brought into the API repository so that the headers, the
> doc-derived import libraries and the startup layer that consumes them
> are versioned, built and verified together.  The upstream repository is
> kept as-is for now; this copy is the one `make crt` and `make e2e`
> build.  Paths are unchanged apart from the `crt/` prefix, so
> `src/crt/crt0.c` is now `crt/src/crt/crt0.c`.
