/*
 * Bt_buffer.h -- the header spelling the official CE pages print for
 * the Bluetooth HCI buffer macro.
 *
 * Copyright (c) 2026 Akari API contributors
 * SPDX-License-Identifier: MIT
 *
 * 1 official page name Bt_buffer.h in their Header row. Every
 ...
 *
 * A consumer that writes `#include <Bt_buffer.h>` -- the spelling the
 * documentation uses -- therefore gets exactly the declarations
 * the documentation attributes to it.
 *
 *   BD_BUFFER (ms920205) -- Bt_hcip.h
 *       Windows CE .NET 4.0 and later.
 */

#ifndef AKARI_BT_BUFFER_H_
#define AKARI_BT_BUFFER_H_

#include "Bt_hcip.h"

#endif /* AKARI_BT_BUFFER_H_ */
