...
export list against what the public header declares. The public headers used for
the arithmetic are mingw-w64's, whose headers are offered as public domain.
...
**Four modules are reachable; six are not.** `urlmon`, `dnsapi`, `schannel` and
`shlwapi` -- 151 exports -- have CE exports that a public domain desktop header
fully or almost fully declares, so CE's `.def` can decide which names exist and
the public header can supply the signatures. The other six -- `btdrt`,
`cellcore`, `uspce`, `msmqrt`, `sim`, `sms`, 178 exports -- are CE-only modules
that no desktop toolchain ever covered.
...
`hughbe/windows-sdk-headers` was also checked and is not usable here: it holds
desktop SDK headers only (10.0.17763 through 10.0.22557), no CE headers.
